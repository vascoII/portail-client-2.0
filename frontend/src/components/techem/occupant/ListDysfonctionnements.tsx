...
import { useDysfonctionnements } from "@/lib/hooks/useDysfonctionnements";
...
export default function ListDysfonctionnements({ fkUser }: ListDysfonctionnementsProps) {
  const { getDysfonctionnements, exportDysfonctionnements } = useDysfonctionnements(fkUser);
  ...
  const handleExportDysfonctionnementsExcel = useCallback(async () => {
    await exportDysfonctionnements();
  ...
  useEffect(() => {
    ...
    const loadDysfonctionnements = async () => {
      try {
        setIsLoading(true);
        const response = await getDysfonctionnements();
        if (!isMounted) {
          return;
        }
