  ...
  const { categories, points } = useMemo(() => {
    ...
    const directSerie =
      logement && typeof logement === "object" && "LogementEF" in logement
        ? extractValeursXYL((logement as Record<string, unknown>).LogementEF)
        : undefined;
    ...
    const nestedLogement =
      logement && typeof logement === "object" && "Logement" in logement
        ...
        : undefined;
    const nestedSerie =
      nestedLogement && typeof nestedLogement === "object" && "LogementEF" in nestedLogement
        ? extractValeursXYL((nestedLogement as Record<string, unknown>).LogementEF)
        : undefined;

    // Fallback: consoTabs.EF.ListeInfosAppareils.infosAppareilEAU[0].SerieConsos.ValeursXYL
    const consoTabs =
    ...
    const efTab =
      consoTabs && typeof consoTabs === "object" && "EF" in consoTabs
        ? (consoTabs.EF as Record<string, unknown> | undefined)
        : null;

    const listeInfosAppareils =
      efTab && typeof efTab === "object" && "ListeInfosAppareils" in efTab
        ? (efTab.ListeInfosAppareils as Record<string, unknown> | undefined)
        : null;
    ...
    return (
      <div className="rounded-xl border border-[#1d1914] bg-white px-5 pb-5 pt-5 shadow-[0_0.625rem_0.938rem_0_rgba(0,0,0,0.2)] sm:px-6 sm:pt-6">
        <h3 className="text-xl font-normal text-[#1d1914]">
          Evolution des index Compteur Eau froide Conso Série
        </h3>
