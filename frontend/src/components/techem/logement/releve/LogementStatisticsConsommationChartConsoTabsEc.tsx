...
export default function LogementStatisticsConsommationChartConsoTabsEc({ pkLogement }: LogementStatisticsChartProps) {
  ...
  const { categories, points } = useMemo(() => {
    ...
    const rawValues = logementData?.consoTabs?.EC?.EvolutionChartData?.data;
    ...
    return (
      <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
          Evolution des index Compteur Eau chaude Conso
        </h3>
