...
const parseValeursXYLIndexValues = (rawSerie?: string): { categories: string[]; points: ParsedChartPoint[] } => {
  ...
  rawSerie
    ...
    .forEach((segment) => {
      ...
      const label = formatDateLabel(dateRaw);
      if (!label) {
        return;
      }
...
export default function LogementStatisticsConsommationChartConsoTabsEf({ pkLogement }: LogementStatisticsChartProps) {
  ...
  const meters = useMemo(() => {
    ...
    const infos =
      ...
      logementEF.ListeInfosAppareils &&
      ...
      Array.isArray((logementEF.ListeInfosAppareils as Record<string, unknown>).infosAppareilEAU)
        ? ((logementEF.ListeInfosAppareils as Record<string, unknown>).infosAppareilEAU as Array<Record<string, unknown>>)
        : [];
    ...
    return infos
      .map((item) => {
        ...
        const isEf = fluide === "EF" || typeAppareil === "EF";
        if (!isEf || !numero) {
          return null;
        }
  ...
  const { categories, points } = useMemo(() => {
    ...
    const rawValues = logementData?.consoTabs?.EF?.EvolutionChartData?.data;
    ...
    return (
      <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">
          Evolution des index Compteur Eau froide Conso
        </h3>
