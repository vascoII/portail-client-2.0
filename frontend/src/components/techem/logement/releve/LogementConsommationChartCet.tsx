...
interface LogementConsommationChartCetProps {
  pkLogement: string;
}
...
export default function LogementConsommationChartCet({ pkLogement }: LogementConsommationChartCetProps) {
  ...
  const { categories, values } = useMemo(() => {
    ...
    const logementCET =
      logement && typeof logement === "object" && "LogementCET" in logement
        ? (logement.LogementCET as Record<string, unknown> | undefined)
        : null;

    const consoPeriode =
      logementCET && typeof logementCET === "object" && "ConsoPeriode" in logementCET
        ? (logementCET.ConsoPeriode as Record<
            "R1" | "R2" | "R3" | "R4" | "R5",
    ...
    const serieConsosDJU =
      logementCET && typeof logementCET === "object" && "SerieConsosDJU" in logementCET
        ? (logementCET.SerieConsosDJU as { ValeursXYL?: string } | undefined)
        : null;
  ...
  const options: ApexOptions = useMemo(() => {
    return {
      colors: ["#417232"], // Techem green for energy counter
      chart: {
