...
export const ChantierImmeubleCard = ({ pkImmeuble }: ImmeubleRelevesCardProps) => {
  const { useImmeubleQuery } = useImmeubles();
  const { data: immeubleData, isLoading: isImmeubleLoading } = useImmeubleQuery(pkImmeuble ?? -1);
  
// Extract immeuble information from API response
  const chantierInfo = useMemo(() => {
    const chantier = immeubleData?.chantier;
    return {
