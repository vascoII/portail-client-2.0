...
SQL,
        ...
        if ($pkImmeubles !== []) {
        // --- FIX ORA-01795 : découpe automatique en blocs de 1000 valeurs ---
        $chunks = array_chunk($pkImmeubles, 1000);
        $whereParts = [];
        $params = [];

        foreach ($chunks as $chunkIndex => $chunk) {
            $placeholders = [];
            foreach ($chunk as $idx => $pk) {
                $paramName = ":imm_{$chunkIndex}_{$idx}";
                $placeholders[] = $paramName;
                $params[$paramName] = (int)$pk;
            }
            $whereParts[] = "fkimmeuble IN (" . implode(",", $placeholders) . ")";
        }

        $whereClause = implode(" OR ", $whereParts);

// --- SQL final avec gestion illimitée des immeubles ---
$sqlRatio = <<<SQL
SELECT 
